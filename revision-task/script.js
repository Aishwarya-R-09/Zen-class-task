...
var Pagination = {
    ...
    Last: function() {
        Pagination.code += '<i class="text-white">...</i><a>' + Pagination.size + '</a>';
    },
...
async function getData(page){
    ...
    for(let i=0; i<n; i++){
        ...
        h3.className = "card-title bg-success p-3 text-white"
        let body = document.createElement("div");
        body.className = "card-body row"
        h3.innerHTML = data[i].name ? data[i].name: "Unknown Character ";
        div.append(h3);
        for(let j in data[i]){
            if(j=="name")
                continue;
